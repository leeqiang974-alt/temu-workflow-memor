...
(function () {
  ...
  async function waitForProductTableChanged(beforeSignature, beforePage, timeoutMs) {
    ...
    while (Date.now() - start < timeoutMs) {
      ...
      // Only consider the page loaded when actual product rows exist AND the
      // content signature differs from the previous page. A page-number change
      // alone is NOT proof — Temu can advance the page counter while serving
      // stale data (observed beyond page 100, 195 total pages).
      if (visibleCount > 0) {
    ...
    console.warn("[TemuFilter v9] wait page load timeout (content unchanged — possible stale page beyond 100)");
    return false;
  }
  ...
  async function globalScanAllPages(minPct, maxPct, onProgress) {
    ...
    for (var loopPage = 1; loopPage <= GLOBAL_SCAN_MAX_PAGES; loopPage++) {
      ...
      // Stale-page detection: if table content is identical to the previous
      // page, Temu is serving stale data (observed beyond page 100). Stop
      // after 2 consecutive stale pages to avoid wasting time on duplicates.
      var contentSig = getTableSignature();
      if (contentSig && contentSig === prevContentSig) {
        stalePageCount++;
        console.warn("[TemuFilter v9] stale page detected:", currentPage, "same content as previous (stale count: " + stalePageCount + ")");
        if (stalePageCount >= 2) {
          console.warn("[TemuFilter v9] stop scan: 2 consecutive stale pages at page", currentPage, "(Temu data cap suspected)");
          if (typeof onProgress === "function") onProgress({ page: currentPage, totalPage: totalPage, total: records.length, stoppedByStale: true });
          break;
        }
      } else {
        stalePageCount = 0;
      ...
      if (!moved) {
        console.warn("[TemuFilter v9] stop global scan: next page content unchanged (stale page / Temu data cap at page", currentPage + ")");
        if (typeof onProgress === "function") onProgress({ page: currentPage, totalPage: totalPage, total: records.length, stoppedByStale: true });
        break;
      }
  ...
  function inferStoreFromPage() {
    var visibleText = document.body && (document.body.innerText || document.body.textContent) || "";
    var domText = document.documentElement && document.documentElement.innerHTML || "";
    return /yeahf/i.test(visibleText) || /yeahf/i.test(domText) ? "YeahF" : "";
  }
    ...
    globalScanBtn.addEventListener("click", async function () {
      ...
      try {
        var records = await globalScanAllPages(minV, maxV, function (progress) {
          if (progress.stoppedByStale) {
            count.textContent = "检测到 Temu 数据停滞（第 " + progress.page + " 页内容与上一页相同），已停止并导出已扫描结果。Temu 可能限制了超过 100 页的数据，请缩小筛选范围分批扫描。";
            return;
          }
