...
SECTIONS = [
    ...
    (
        ...
        [
            "同一精确 D 的标题必须完全一致。",
            "标题重构优先 70-80 个中文字符，首部保留数量/规格信息，例如 1件、1套、三件装、50个、15/30、2/3层。",
            "标题必须准确描述品类和可见产品，不写图片/来源不支持的功能或卖点。",
            "标题禁止使用自动、智能、AI、APP、蓝牙、wifi、遥控、感应、电动、充电、电池、USB、儿童、玩具、环保、有机、可降解、食品级、最佳、最好、顶级等高风险词。",
            "每个 D 必须带确定性追踪码/指纹，例如字母-数字-字母格式，用于回查原始行。",
            "标题追踪码/指纹按文件、按精确 D 绑定；不同文件的同一 D 可以有不同标题指纹。",
            "后续只修 T/J/U 时，不得重新生成标题；应从原始最终确认表按精确 D 拿回带追踪码的重构标题。",
            "不得从过程表、复检前表、候选表、失败表读取标题指纹作为最终入库依据。",
            "最终校验必须检查 same-D title mismatch、标题长度、禁词、标题指纹缺失/重复。",
        ],
    ),
    (
        "其他格子匹配与审查",
        [
            "处理前必须快照有效行的 row、D、G、SKU、标题、J、T、U，以及店铺辅助列。",
            "行数和行顺序必须保持不变，除非用户明确要求重排行。",
            "D、G、SKU、变体/规格列属于行身份字段，默认禁止改动。",
            "普通 T/J/U/标题回传只允许改任务指定列，通常是 产品标题、预览图(J)、轮播图(T)、产品素材图(U)。",
            "公式列、价格列、库存列、申报信息列、未授权店铺字段不得被顺手改动。",
            "X/AC 等店铺辅助列必须按最新店铺规则校验；只有规则明确要求时才允许更新。",
            "更新 X/AC 或其他辅助列时，必须输出逐行 before/after diff。",
            "J 按行匹配，T/U/标题按精确 D 共享；不能把 D 级字段误写成行级，也不能把行级 J 误写成 D 级。",
            "最终表必须输出 changed-cell diff summary：row、header、old value、new value。",
            "任何未授权列变化、行身份漂移、公式丢失、格式/类型异常，都是交付阻断错误。",
        ],
    ),
    (
        "T 首图",
