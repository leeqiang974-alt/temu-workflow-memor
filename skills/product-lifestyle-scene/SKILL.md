...
- 2026-07-24 batch-role isolation correction: a new image-generation task is not automatically a workbook-writeback redo, even when it has the same D count or product families. Persist an explicit `batch_role` (`workbook_writeback`, `redo_for_same_workbook`, or `reusable_material_library`) plus `authorized_workbook` and `writeback_forbidden_for`. A final writer must consume the exact user-approved batch path and manifest SHA-256; never choose the latest directory, newest timestamp, or another “289D” batch by resemblance.
