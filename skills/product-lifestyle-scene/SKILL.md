...
- **L043 1999D human-review correction (2026-08-13)**: the user confirmed that rejected L043 candidates changed the sale-product appearance. For every rejected exact D, use the current-workbook T1 only for scene/composition and supply a visually verified exact-D product material as a second reference. Lock board outline, panel count, hole pattern, supports/raised details, color, quantity/spec and proportions; edit only non-product surroundings. Never use a rejected candidate as the next reference.
...
- **L072 1999D human-review correction (2026-08-13)**: the user confirmed that rejected L072 candidates invented or changed the product appearance. Redo from the current-workbook T1 scene anchor plus a visually verified exact-D product material. Preserve exact drawer/layer count, front-hole/handle pattern, rods, connector rings, top handles, feet, color and proportions. A coherent but newly imagined cabinet is a hard reject.
- **L081 1999D wrong-inheritance lock (2026-08-13)**: rejected L081 candidates frequently inherited the wrong sale-product appearance. Never resolve product identity from a sibling D, generic same-L0xx image, title, or the foreground product in an unreliable T1. Resolve each exact D independently using visually verified row-level `D + G + SKU` product material and pass that material with the current-workbook T1 scene/composition anchor. Reject any output whose product appearance does not match that exact material, even if the scene is attractive.
...
- **L068 1999D appearance/proportion correction (2026-08-13)**: the user confirmed that rejected L068 candidates changed product appearance and sometimes produced incorrect proportions. Redo only from the current-workbook T1 scene anchor plus visually verified exact-D product material. Lock silhouette, component relationships and width/height/depth ratios; reject stretched, compressed, widened or narrowed products even when they look plausible.
- **L082 1999D wrong-appearance correction (2026-08-13)**: the user confirmed that rejected L082 candidates show the wrong product appearance. Lock the complete body, expandable mechanism, supports, surfaces and proportions against exact-D verified material. A generic shelf or similar organizer is a hard reject.
...
- **L083 1999D detail correction (2026-08-13)**: the user confirmed that rejected L083 candidates have product-appearance detail errors. Rods, metal plates/sheets, supports, connectors, frame geometry and visible hardware are exact identity evidence; none may be added, removed, merged or simplified.
...
- **L091 1999D detail correction (2026-08-13)**: the user confirmed that rejected L091 candidates have product-appearance detail errors. Preserve the top structure, groove/recess pattern, transparent front, handle, frame and upper-edge geometry exactly. A small missing, added or reshaped detail is a hard reject, not a cosmetic issue.
