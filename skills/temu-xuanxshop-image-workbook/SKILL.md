...
5. **Generate/edit T first images**
   ...
   - L043 source-quality gate: do not use scene screenshots, images with folded clothing covering the product, text labels such as `pcs`, heavy background, blurry edges, or non-cutout lifestyle photos as direct image2 product references. These make image2 treat the input as a scene/concept and redraw an unrelated folding board. For L043 redo, use a clean high-contrast cutout/product-only source, or generate a scene separately and composite the fixed product PNG.
