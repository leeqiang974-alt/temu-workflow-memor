...
- Badge shape is batch-specific. Do not carry an older circular-inset rule into a later workbook when the user approved a square mode. For the YeahF interleaved 2000D batch, use a roughly 22%-width square inset with contain/no-crop fitting and large outlined `THIS IS THE PRODUCT` text; no circle and no arrow. Preserve safe-corner occlusion checks and bind any inherited pre-interleave badge candidate to the recomputed final D before approval or writeback.
- A consolidated J lineage manifest is preparation, not approval. For the YeahF interleaved 2000D batch, all 3,690 physical rows must remain blocked until a side-by-side review keyed by final `D + G + SKU` explicitly confirms the inspected source, local/generated J, written URL, and AC preview linkage. Do not convert a non-empty URL, same-D inheritance, or AC equality into a visual pass.
- Treat G as a row-level variant fact, not a product-use marketing description. Preserve source color, count, size, material, and set claims exactly. For differentiation, add only neutral style wording; never append scene/use phrases that are not variant attributes. When G changes, update E, the first SKU JSON `specName`, J, SKC `previewImgUrls`, SKC `extCode`, and model fields as one linked unit.
- For a new D cloned from an already user-approved supplement D, prefer exact `clone_source_D + SKU` as the J authority. Do not rotate J across same-`L0xx` rows merely for visual variety when it weakens color/count/spec provenance. Any alternative source must prove matching canonical tokens and retain L043 quantity/color and L095 2-vs-3 visual contracts.
