...
class ReleaseGuard:
    ...
    def init_batch(self, batch_id: str, workbook: str | Path) -> dict[str, Any]:
        ...
        self.db.execute("INSERT OR REPLACE INTO batches VALUES (?,?,?,?,?,?,NULL)",
                        tuple(row[k] for k in ("batch_id", "workbook_path", "workbook_sha256", "policy_sha256", "created_at", "status")))
        self.db.commit()
    ...
    def evaluate(self, batch_id: str) -> dict[str, Any]:
        ...
        required = ("proposal_confirmation", "execution_receipt", "t1_approval", "badge_approval", "j_audit", "cell_audit", "negative_locks", "writeback_diff", "category_attribute_audit")
        for k in required:
        ...
        return {"batch_id": batch_id, "status": status, "reasons": reasons, "evidence": ev}
